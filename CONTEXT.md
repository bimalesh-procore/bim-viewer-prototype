...
### Wired and working
- **Reset** → `viewer.resetView()`. The right-toolbar reset button has two badge elements:
  - **ModificationPill** — orange (`#FF5100`) pill that pops in with a descriptive label ("Objects Hidden", "Sectioning Applied", "Objects Isolated", "Measurements Added") for 3 seconds after any modification fires, then collapses (max-width + opacity transition, ~350ms) and hands off to the dot. The label re-fires on sectioning-mode exit so it always plays even when the pill was hidden during the session. Pill position: `-translate-y-[135%] translate-x-[15%]` relative to the reset button.
  - **ModificationDot** — persistent 6px orange circle at `top: 3px, left: 3px` (inside the button's top-left corner). Appears with a spring pop-in (`mv-badge-pop`) ~300ms into the pill collapse so the two overlap briefly, creating the illusion of the pill becoming the dot. Disappears with `mv-badge-pop-out` when all modifications are cleared via Reset.
  - **View Modifications flyout** — opens on button click. Header: "View Modifications" title + "Reset" button. Body: flat list of category filter tokens (Objects Hidden, Sectioning, Measurements), each with a blue toggle to temporarily suspend/resume that category and an × to clear it entirely. Toggle OFF for "Objects Hidden" saves the hidden list and calls `showAll()`; toggle ON restores it. Sectioning toggle calls `setPlaneEnabled(id, false/true)` on each clip plane. New adapter method: `setActionCategoryEnabled(category, enabled)`. `ActionHistorySummary` gains `disabledCategories: ActionHistoryCategory[]`.
...
- **Search Sets panel** → full panel for creating, importing, managing, and running named search sets (groups of BIM elements identified by GUID). Key interactions:
  - **Import from XML** — parses Navisworks-exported `.xml` files (`src/chrome/features/search-sets/xmlParser.ts`). Preview tree lets users pick which sets to import; duplicate-detection shows a modal for each collision (Skip / Overwrite). "Import from Navisworks" in the `+` menu opens a file picker directly.
  - **Create Folder** — `+ → Create Folder` creates a named empty folder (manual/extra folder with a `__newfolder__` ID prefix). Immediately enters inline rename mode.
  - **Tree rendering** — sets are grouped by source file then nested by `folderPath` into a `ListFolder`/`ListLeaf` tree. Manual empty folders interleave via `parentId`. The tree is built in `buildListTree` + `buildListFolder` helpers in `panelContent.tsx`.
  - **Inline rename** — double-click or Rename in the per-row overflow renames both derived folder labels (`onRenameFolder`) and manual folders (updates local state + `onSaveFolder`).
  - **Create nested folder** — per-row overflow "Create new folder" on folder rows creates a child `__newfolder__` under that folder (`parentId` wired).
  - **Drag-and-drop reorder** — native HTML5 DnD via `TreeNode`'s built-in `draggable` / `onDrag*` props. `resolveDrop`, `validateMove`, and `performMove` in `panelContent.tsx` manage position (before/after/inside). Order persists via `SearchSetStorage.saveOrder()` → `localStorage`. Sets may only be reordered within derived-folder siblings; extra folders can nest anywhere.
  - **Run search** — hover actions (magnifying glass icon on folder and set rows) or the footer "Search" button send GUIDs to `adapter.runSearchSet()` which calls `SearchQueryEngine.js` to select matching elements. Bulk selections run multiple sets. Toast shows "X objects found."
  - **Bulk actions** (Action dropdown, visible when any items are checked):
    - **Delete** — bulk-deletes selected sets and folders with a confirmation modal.
    - **Move to folder → New Folder** — creates a new derived/manual folder and moves selected items into it. Fully-selected derived sub-trees are moved as whole folders via `onMoveFolder`; individual sets use `onMoveSet`.
    - **Move to folder → Existing Folder** — opens `MoveToFolderModal` (folder-picker dialog). Shows all current folders; selecting one and clicking "Move" calls `onMoveSet` for sets or updates `parentId` for extra folders.
  - **Per-row overflow** (⋯ button): Rename, Create new folder (folders only), Move to folder (New or Existing), Delete.
  - **Persistence** — search sets (name, source, folderPath, guids) live in `SearchSetStorage.js` → `localStorage`. Manual empty folders and their `parentId` hierarchy persist separately. Drag-order persists as a `Record<parentKey, string[]>` order map. Adapter exposes `getSearchSets`, `saveSearchSet`, `deleteSearchSet`, `getSearchSetFolders`, `saveSearchSetFolder`, `deleteSearchSetFolder`, `getSearchSetOrder`, `saveSearchSetOrder`.
  - **Search filter bar** — query filters the tree live (debounced); matching subtrees are highlighted; auto-expands all matching folders while query is active. State: `useSearchSetsQuery` (`useSyncExternalStore` singleton in `src/chrome/shared/useSearchSetsView.ts`).
  - **Empty state** — shown with an illustration when no sets have been imported yet.
  - **`MoveToFolderModal`** — portal modal (matching Figma node 759-278928): header with folder icon, scrollable folder-only tree with expand/collapse chevrons, row selection highlight `#EDF2FC`, Cancel + primary orange Move button. Top-level folders are expanded by default; clicking the scrim closes.

### Wired to stubs (engine feature not built)
- Properties, Measure — log to console

### Not wired yet
- Deviation, Render Modes (mesh/lines/terrain/point-cloud toggles — context-only, no engine concept), Markup, Quick Create, ViewCube faces, Header Search, MiniMap
